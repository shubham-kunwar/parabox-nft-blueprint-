...
use scrypto::prelude::*;
#[blueprint]
mod metakul_nft_blueprint {
    use super::*;

    struct MetakulNFT {
                /// The resource address of all random cards
                metakul_nft_resource_manager: ResourceManager,
    ...
    impl MetakulNFT {
        pub fn instantiate_component() -> Global<MetakulNFT> {

            let special_cards_bucket = ResourceBuilder::new_string_non_fungible(OwnerRole::None)

                .metadata(metadata!(
                    init {
                        "name" => "Metakul Nft".to_owned(), locked;
                    }
                ))
            ...
            let metakul_nft_resource_manager =
                ResourceBuilder::new_integer_non_fungible::<MetakulMetadata>(OwnerRole::None)
                    .metadata(metadata!(
                        init {
        ...
        pub fn mint_metakul_nft(&mut self) -> Bucket {
            ...
            let nft_bucket = self.metakul_nft_resource_manager.mint_non_fungible(
